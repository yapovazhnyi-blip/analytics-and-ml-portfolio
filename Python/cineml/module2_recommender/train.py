...
class ImplicitDataset(Dataset):
    ...
    def __init__(
        ...
        for _, row in movies.iterrows():
            raw = row.get("genres", [])
            # genres may be a numpy array, Python list, or None after Parquet round-trip
            if raw is None:
                genres = []
            elif hasattr(raw, "tolist"):
                genres = raw.tolist()   # numpy array → list
...
def train_two_tower(ratings: pd.DataFrame, movies: pd.DataFrame, epochs: int = 20) -> None:
    ...
    # Save config so the interface can reconstruct the exact architecture
    cfg = {"n_users": n_users, "n_items": n_items, "n_genres": n_genres,
           "d_embed": 64, "d_model": 128}
    (MODEL_DIR / "two_tower_config.json").write_text(
        json.dumps(cfg, indent=2), encoding="utf-8"
    )
    log.info("Two-Tower model saved. Config: %s", cfg)
